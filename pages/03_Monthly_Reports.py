...
# -----------------------------------------------------------------------
@st.cache_data(ttl=1800, show_spinner="Loading data from BigQuery...")
...
# HEADER
# -----------------------------------------------------------------------
st.title(f"Monthly Report -- {sel_label}")

ssl_icon = "\U0001f7e2" if ssl_pct >= 80 else ("\U0001f7e1" if ssl_pct >= 30 else "\U0001f534")
st.markdown(f"""
<div class="alert-bar">
  <strong>{ssl_icon} Overall SSL: {ssl_pct:.1f}%</strong> &nbsp;&middot;&nbsp;
  PO Value: <strong>{po_total:,.0f} KD</strong> &nbsp;&middot;&nbsp;
  Received: <strong>{recv_total:,.0f} KD</strong> &nbsp;&middot;&nbsp;
  Gap: <strong>{gap_total:,.0f} KD</strong> &nbsp;&middot;&nbsp;
  {vendor_count} vendors &nbsp;&middot;&nbsp; {item_count:,} SKUs &nbsp;&middot;&nbsp; {zero_count:,} undelivered SKUs
</div>
""", unsafe_allow_html=True)
...
k3.metric("Gap",              f"{gap_total/1_000:,.1f}K KD")
...
# -- SSL colour helper (works for both pandas .map and .applymap) -------
def _ssl_style(val):
...
# Use .map() (pandas >= 2.1) with fallback to .applymap() for older pandas
def _apply_style(styler, fn, subset):
    try:
...
with tab_lines:
    ...
    if po_csv.exists():
        ...
        def _load_lines(month: str):
            raw = pd.read_csv(po_csv, parse_dates=["Created Date"])
            raw["month"] = raw["Created Date"].dt.to_period("M").astype(str)
            m = raw[(raw["month"] == month) & (raw["Quantity"] > 0)]
        ...
        if lines_raw.empty:
            ...
            keep = {s: d for s, d in col_map.items() if s in lines_raw.columns}
            lines_df = (
            ...
            st.markdown(
                f"**{len(lines_df):,} individual PO lines** with zero received qty "
                f"in {sel_label}."
            )
            ...
            if srch2:
                ...
                for col in ["PO Number", "Item No.", "Item Name", "Vendor",
                            "Category", "Brand", "Line Status"]:
            ...
            def _status_style(val):
                return ("background-color:#FFFBEB; color:#92400E"
                        if str(val).strip().lower() == "open" else "")
        ...
        st.info(
            "**Item-level PO lines** are not stored in BigQuery -- they require the daily "
            "sync CSV (`po_latest.csv`) which is only available when the dashboard runs "
            "locally with the Shared Drive mounted.\n\n"
            "**To access item-level detail:**\n"
            "- Download the pre-built Excel from the **Download Excel** tab -- "
        ...
        st.markdown("---")
        st.markdown(f"**Closest available: aggregated undelivered SKUs from BigQuery "
                    f"({len(sku_df):,} rows)**")
        st.dataframe(
...
with tab_dl:
    ...
    if st.button("Generate Excel", key="gen_xl"):
        with st.spinner("Building Excel..."):
            buf = io.BytesIO()
            with pd.ExcelWriter(buf, engine="xlsxwriter") as writer:
                wb  = writer.book
                hdr = wb.add_format({
                    "bold": True, "bg_color": "#0D1520", "font_color": "#FFFFFF",
                    "border": 1, "align": "center", "font_name": "Calibri", "font_size": 10,
                })
                grn = wb.add_format({
                    "num_format": "0.0%", "border": 1, "font_name": "Calibri", "font_size": 10,
...
> The pre-built Excel (when available) also includes a *Zero-Received Lines* sheet
